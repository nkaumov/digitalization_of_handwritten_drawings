...
- AI-ориентированные и расширенные сценарии редактора следующих этапов (автоимпорт результата, продвинутая ручная коррекция, расширение геометрии).

## Что готово по этапу 6 (факт)

- Реализована стартовая страница со списком чертежей и переходом в редактор.
- Реализованы viewport, сетка, панорамирование и экранный зум.
- Реализованы визуальное рабочее поле, слой сегментов и слой точек.
- Реализована отрисовка нескольких контуров.
- Реализованы выделение сегмента и выделение его конечных точек/ручек, включая множественное выделение сегментов.
- Реализовано ручное создание линии и добавление прямоугольного контура по размерам.
- Реализованы перемещение конечных точек/ручек линий и перемещение сегмента/группы сегментов.
- Реализовано удаление сегмента.
- Реализованы соединение и разъединение сегментов через общую точку.
- Реализованы базовая проверка геометрии и подсветка warning-ов.
- Реализованы отображение числовых размеров возле сегментов и редактирование длины/угла выбранного сегмента.
- Реализованы боковая панель инструментов и верхние панели управления.
- Реализованы загрузка чертежа из backend и сохранение чертежа в backend.
- Реализована базовая история действий: undo (`Ctrl+Z`).
- Зафиксирована line-first модель UX:
  - пользовательская первичная сущность — линия/сегмент;
  - точки не создаются и не удаляются как отдельная пользовательская сущность;
  - точки используются как концы/ручки сегментов для hover/edit/connect.

## Что осталось до формального закрытия этапа 6

- Этап 6 закрыт по факту в актуальной line-first постановке.
- Дальнейшие UX-улучшения и стабилизация edge-cases относятся к следующим этапам улучшения редактора.

## Что готово по этапу 7 (факт)

- Кнопка создания нового чертежа и модалка выбора сценария (`пустой` / `по фото`) работают в текущем home-flow.
- Текущий `create blank` flow и переход в редактор сохранены без изменений.
- Активирован photo-flow в create-модалке с отдельным экраном загрузки изображения.
- Добавлена инструкция по качеству фото в модалке загрузки.
- Добавлена базовая клиентская проверка файла (выбор файла, тип `image/*`, лимит размера).
- Реализована отправка изображения в backend (`/api/v1/files/source-image`).
- Реализован запуск распознавания, polling статуса и получение результата через backend placeholder-flow (`/api/v1/recognition-jobs/*`).
- Добавлен визуальный статус обработки (upload/start/process/result/open).
- Добавлена обработка ошибок загрузки и распознавания с пользовательскими сообщениями.

## Статус этапа 7

- Этап 7 закрыт по факту в границах текущего backend placeholder-flow.

## Что готово по этапу 8 (факт)

- Добавлен endpoint приема задания на распознавание в `apps/ai-service`.
- Подготовлен вход в pipeline runner и согласованный контракт запроса/ответа на уровне AI-service.
- Реализована система конфигурации порядка стадий и отключения стадий через env.
- Реализован последовательный запуск стадий с логированием и сбором служебных данных.
- Реализовано сохранение debug-артефактов в локальное хранилище.
- Добавлен локальный CLI для тестового прогона пайплайна и пошаговой отладки.
- Подготовлена структура для подмены/расширения реализаций стадий без изменения раннера.
