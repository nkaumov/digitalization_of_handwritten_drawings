...
- Этап 10 частично закрыт: базовая детекция и граф есть, но качество на реальных фото требует доработки.
...
- Доработка качества line detection/graph на реальных фото (этап 10).
- Реальная детекция текста и OCR (этапы 11+).
- Улучшение качества line detection и устойчивости графа (этап 10 в части качества).
...
## Что готово по этапу 10 (line detection + graph)
...
- Этап 10 не закрыт: требуется улучшить качество детекции линий на реальных фото (например, исключение границ кадра и фильтрация шумов).
