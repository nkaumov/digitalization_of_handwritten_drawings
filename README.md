...
- Этап 6 выполнен частично: реализован рабочий каркас редактора и большая часть задач, но этап формально не закрыт.
...
- Реализованы выделение сегмента и выделение точки, включая множественное выделение сегментов.
- Реализовано ручное создание линии и добавление прямоугольного контура по размерам.
- Реализованы перемещение точки и перемещение сегмента/группы сегментов.
...
- Доделать явное удаление точки в допустимом сценарии как отдельную пользовательскую операцию (пункт этапа 6.15 в `docs/dev_plan.md`).
- Провести стабилизацию edge-cases ручного редактирования (стыковка/фиксация длины) и финальную приемку пункта "рабочий ручной редактор".
