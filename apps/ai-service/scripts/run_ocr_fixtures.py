...
def _report_path(reports_dir: Path, suffix: str) -> Path:
    ...
    return reports_dir / f"ocr-report-{timestamp}.{suffix}"
...
def _build_manager(outputs_dir: Path):
    ...
    config = PipelineConfig(
        stage_order=list(OCR_STAGE_ORDER),
    ...
    return PipelineManager(
        ...
        default_triggered_by="fixtures.ocr",
    )
...
def _write_markdown(report_path: Path, report: Dict[str, Any]) -> None:
    lines = [
        ...
        "",
        "| id | image | expected | candidates | suspicious | confidence(avg raw) | status |",
        "| --- | --- | --- | --- | --- | --- | --- |",
    ]
    for item in report["items"]:
        ...
        row = [
            ...
            ", ".join(map(str, item.get("candidates", []))) or "-",
            str(item.get("suspicious", 0)),
            f"{item.get('confidence_raw_avg', 0):.1f}"
            if item.get("confidence_raw_avg") is not None
            else "-",
            item.get("status", "-"),
        ]
...
def run(manifest_path: Path, outputs_dir: Path, reports_dir: Path) -> int:
    ...
    for item in items:
        ...
        result = manager.run(context, triggered_by="fixtures.ocr")
        meta = result.context.get("meta", {})
        candidates = []
        for item_value in meta.get("ocr_candidates", []):
            candidates.extend(item_value.get("values", []))

        suspicious = meta.get("ocr_suspicious", [])
        candidate_confidences = [
            item_value.get("confidence")
            for item_value in meta.get("ocr_candidates", [])
            if item_value.get("confidence") is not None
        ]
        confidence_avg = (
            sum(candidate_confidences) / len(candidate_confidences) if candidate_confidences else None
        )
        raw_confidences = [
            item_value.get("confidence")
            for item_value in meta.get("ocr_raw", [])
            if item_value.get("confidence") is not None
        ]
        confidence_raw_avg = sum(raw_confidences) / len(raw_confidences) if raw_confidences else None

        expected_match = all(value in candidates for value in expected_values)
        status = "ok" if expected_match else "mismatch"
        ...
        report_items.append(
            {
                ...
                "confidence_raw_avg": confidence_raw_avg,
                "status": status,
            }
        )
...
def main(argv: List[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run OCR fixtures")
    parser.add_argument(
        "--manifest",
        default=str(APP_ROOT / "ocr_fixtures" / "manifest.json"),
        help="Path to manifest.json",
    )
    parser.add_argument(
        "--outputs-dir",
        default=str(APP_ROOT / "ocr_fixtures" / "outputs"),
        help="Directory for outputs",
    )
    parser.add_argument(
        "--reports-dir",
        default=str(APP_ROOT / "ocr_fixtures" / "reports"),
